...
public class DBSpeedComparison {
	...
	static String anchorLookup = "Elisabeth";
	static String semSigLookup = "Germany";

	public DBSpeedComparison() {
		// TODO Auto-generated constructor stub
	}
	
	public static void semSigTest(){
		try {
			Stopwatch sw = new Stopwatch(Stopwatch.UNIT.MILLISECONDS);
			LinkedList<String> listH2, listNeo4j;
			System.out.println("Performing test for '" + semSigLookup + "'");
			
			// H2
			sw.start();
			String dbPathH2 = "E:/Master Project/data/H2/AnchorsPlusPagelinks/h2Anchors";
			String partialAnchorSQL = "select ENTITY from ENTITYID where ID in (select ENTITYTOENTITY.ENTITYSINKID from ENTITYID, ENTITYTOENTITY where ENTITYID.ID = ENTITYTOENTITY.ENTITYSOURCEID and ENTITYID.ENTITY is (?))";
			H2Connector semSigH2 = new H2Connector(dbPathH2, "sa", "", partialAnchorSQL);
			sw.stop();
			System.out.println("H2 DB startup time: " + sw + " ms");
			
			// test
			sw.start();
			listH2 = semSigH2.getFragmentTargets(semSigLookup);
			sw.stop();
			...
			// Neo4j
			sw.start();
			GraphDatabaseService graphDB = new GraphDatabaseFactory().newEmbeddedDatabase("../../data/DBs/PageLinksWithWeights");
			Neo4jCore.registerShutdownHook(graphDB);
			Neo4jConnector semSigNeo4j = new Neo4jConnector(graphDB, Neo4jCore.wikiLinkLabel, null);
			sw.stop();
			System.out.println("Neo4j DB startup time: " + sw + " ms");

			sw.start();
			listNeo4j = semSigNeo4j.getFragmentTargets(semSigLookup);
			sw.stop();
	...
	public static void anchorTest(){
		try {
			Stopwatch sw = new Stopwatch(Stopwatch.UNIT.MILLISECONDS);
			LinkedList<String> listH2, listNeo4j;
			System.out.println("Performing test for '" + anchorLookup + "'");
			...
			// test
			sw.start();
			listH2 = partialAnchorsH2.getFragmentTargets(anchorLookup);
			sw.stop();
			System.out.println("H2 Lookup: " + sw + " ms - list lenght: " + listH2.size());
			...
			sw.start();
			listNeo4j = partialAnchorsNeo4j.getFragmentTargets(anchorLookup);
			sw.stop();
			System.out.println("Neo4j Lookup: " + sw + " ms - list lenght: " + listNeo4j.size());
